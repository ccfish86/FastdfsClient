...
public class FastdfsClientImpl extends AbstractClient implements FastdfsClient {
    ...
    public FastdfsClientImpl(List<String> trackerAddrs) throws Exception {
        ...
        trackerClientPool =
                new GenericKeyedObjectPool<String, TrackerClient>(new TrackerClientFactory());
        storageClientPool =
                new GenericKeyedObjectPool<String, StorageClient>(new StorageClientFactory());
        updateStorageIpMap();
    }
    ...
    @Override
    public void close() {
        logger.debug("fastdfs server's pool close.");
        this.trackerClientPool.close();
        this.storageClientPool.close();
    }

    public void clear() {
        logger.debug("fastdfs server's pool clear.");
        this.trackerClientPool.clear();
        this.storageClientPool.clear();
    }
        ...
        try {
            ...
            if (result.getCode() == 0) {
                ...
                Result<String> result2 =
                        storageClient.upload(file, extname, result.getData().getPathIndex());
        ...
        try {
            ...
            if (fileid != null) {
                ...
                if (result.getCode() == 0) {
                    ...
                    storageClient = storageClientPool.borrowObject(storageAddr);
                    Result<String> result2 =
                            storageClient.uploadSlave(file, filename, prefix, ext, null);
                    if (result2.getCode() == 0) {
                        fileId = result2.getData();
                    }
                }
        ...
        try {
            ...
            if (result.getCode() == 0) {
                ...
                for (GroupInfo groupInfo : groupInfos) {
                    Result<List<StorageInfo>> result2 =
                            trackerClient.getStorageInfos(groupInfo.getGroupName());
                    if (result2.getCode() == 0) {
                        ...
                        for (StorageInfo storageInfo : storageInfos) {
                            ...
                            storageIpMap.put(
                                    storageInfo.getIpAddr() + ":" + storageInfo.getStoragePort(),
                                    hostPort);
                        }
                    }
                }
        ...
        try {
            ...
            if (result2.getCode() == 0) {
                ...
                Result<Boolean> result3 =
                        storageClient.setMeta(fastDfsFile.group, fastDfsFile.fileName, meta);
    ...
    public Map<String, String> getMeta(String fileId) throws Exception {
        ...
        try {
            ...
            if (result2.getCode() == 0) {
                storageAddr = result2.getData();
                storageClient = storageClientPool.borrowObject(storageAddr);
                Result<Map<String, String>> result3 =
                        storageClient.getMeta(fastDfsFile.group, fastDfsFile.fileName);
                if (result3.getCode() == 0 || result3.getCode() == 0) {
    ...
    public String getUrl(String fileId) throws Exception {
        ...
        try {
            ...
            Result<String> result =
                    trackerClient.getDownloadStorageAddr(fastDfsFile.group, fastDfsFile.fileName);
            if (result.getCode() == 0) {
                String hostPort = getDownloadHostPort(result.getData());
                // url = "http://"+hostPort+"/" + fastDfsFile.group + "/" + fastDfsFile.fileName;
                url = MessageFormat.format("http://{0}/{1}/{2}", hostPort, fastDfsFile.group,
    ...
    public Boolean delete(String fileId) throws Exception {
        ...
        try {
            ...
            if (result2.getCode() == 0) {
                ...
                storageClient = storageClientPool.borrowObject(storageAddr);
                Result<Boolean> result3 =
                        storageClient.delete(fastDfsFile.group, fastDfsFile.fileName);
                if (result3.getCode() == 0 || result3.getCode() == 0) {
                    result = true;
                }
            }
    ...
    @Override
    public String uploadSlave(BufferFile file, String fileid, String prefix, String ext)
            throws Exception {
    ...
    private String getFileExtName(BufferFile file) {
        String name = file.getName();
        if (name != null) {
            int i = name.lastIndexOf('.');
            if (i > -1) {
    ...
    public BufferFile download(String fileId) throws Exception {
        ...
        try {
            ...
            if (result2.getCode() == 0) {
                storageAddr = result2.getData();
                storageClient = storageClientPool.borrowObject(storageAddr);
                Result<BufferFile> result3 =
                        storageClient.download(fastDfsFile.group, fastDfsFile.fileName);
                if (result3.getCode() == 0 || result3.getCode() == 0) {
                    result = result3.getData();
                }
            }
