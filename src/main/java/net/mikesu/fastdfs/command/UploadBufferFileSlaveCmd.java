 ...
 */
public class UploadBufferFileSlaveCmd extends AbstractCmd<String> {
     ...
     * @param ext 后缀
     */
    public UploadBufferFileSlaveCmd(BufferFile file, String masterfilename, String prefix, String ext){
        super();
        this.file = file;
        this.requestCmd = STORAGE_PROTO_CMD_UPLOAD_SLAVE_FILE;
        this.body2Len = file.getFiledata().length;
        this.responseCmd = STORAGE_PROTO_CMD_RESP;
        this.responseSize = -1;

        byte[] masterfileNameLenByte = long2buff(masterfilename.length());
        byte[] fileSizeLenByte = long2buff(file.getFiledata().length);
